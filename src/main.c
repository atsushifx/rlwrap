...
int   impatient_prompt =
    TRUE; /* show raw prompt as soon as possible, even before we cook it. may result in "flashy" prompt */
char *substitute_prompt = NULL; /* -S option: substitute our own prompt for <command>s */
char *filter_command =
    NULL; /* -z option: pipe prompts, input, output, history and completion requests through an external filter */
...
int   ignore_queued_input =
    FALSE; /* read and then ignore all characters in input queue until it is empty (i.e. read would block) */
...
int we_just_got_a_signal_or_EOF =
    FALSE; /* When we got a signal or EOF, and the program sends something that ends in a newline, take it
...
/* utility functios for private use */
static char *create_history_filename(const char *command_name);
static bool  is_history_file_writable(const char *history_file);
static char *create_completion_filename(const char *command_name);
static char *get_oldtype_filename(const char *command_name, const char *postfix);
static char *get_configdir(const char *appdir, const char *env_name, xdg_type_t xdgtype);
...
int main(int argc, char **argv)
{
    ...
    if (!isatty(STDIN_FILENO) || (getenv("INSIDE_EMACS") && strcmp(getenv("TERM"), "dumb") == 0))
        if (execvp(argv[optind], &argv[optind]) < 0) myerror(FATAL | USE_ERRNO, "Cannot execute %s", argv[optind]);
    ...
    run_unit_test(argc - optind, argv + optind,
                  TEST_AFTER_SPAWNING_SLAVE_COMMAND); /* argv points at the first non-option rlwrap argument */
...
static void fork_child(char *UNUSED(command_name), char **argv)
{
    ...
    if (pid > 0) /* parent: */
        ...
        if (execvp(argv[optind], &argv[optind]) < 0) {
            if (last_opt > 0 && last_option_didnt_have_optional_argument) { /* e.g. 'rlwrap -a Password: sqlpus' will
                                                                               try to exec 'Password:' */
                ...
                fprintf(
                    stderr,
                    "Did you mean '%s' to be an option argument?\nThen you should write -%c%s, without the space(s)\n",
                    argv[optind], last_opt, arg);
            }
            myerror(FATAL | USE_ERRNO, "Cannot execute %s",
                    argv[optind]); /* stillborn child, parent will live on and display child's last gasps */
        }
    }
}
...
void main_loop(void)
{
    ...
    while (TRUE) {
        ...
        DPRINTF2(DEBUG_TERMIO, "calling select() with timeout %s %s ...", timeoutstr,
                 within_line_edit ? "(within line edit)" : "");
        ...
        DPRINTF5(DEBUG_TERMIO, "... returning %d%s %s %s %s", nfds, nfds > 0 ? ": " : ".",
                 ...
                 nfds > 0 && FD_ISSET(master_pty_fd, &writefds)
                     ? "output queue nonempty and pty master ready for output"
                     : "");
        ...
        if (nfds < 0) {                         /* exception  */
            ...
            if (ignore_queued_input) { /* ... we have read all the input keystrokes that should
                ...
                DPRINTF2(DEBUG_SIGNALS, "select returned 0, command_is_dead=%d, commands_exit_status=%d",
                ...
                if (we_just_got_a_signal_or_EOF) {
                    we_just_got_a_signal_or_EOF = FALSE; /* 1. If we got a signal/EOF before cooking time, we don't need
                                                            special action to preserve the cooked prompt.
                                                            2. Reset we_just_got_a_signal_or_EOF  after a signal or EOF
                ...
                if (!skip_rlwrap()) { /* ... or else, it is time to cook the prompt */
                    ...
                    move_cursor_to_start_of_prompt(
                        ERASE); /* cooked prompt may be shorter than raw prompt, hence the ERASE */
                    ...
                    DPRINTF2(DEBUG_READLINE, "After cooking, raw_prompt=%s, cooked=%s", M(saved_rl_state.raw_prompt),
                             M(saved_rl_state.cooked_prompt));
            ...
            /* Always first read and process the slave command's output, even if there is input waiting on stdin
               ...
               c
            */
            if (FD_ISSET(master_pty_fd,
                         &readfds)) { /* there is something (or nothing, if EOF) to read on master pty: */
                nread = read(master_pty_fd, buf, BUFFSIZE - 1); /* read it */
                DPRINTF1(DEBUG_AD_HOC, "nread: %d", nread);
                if (nread <= 0) {
                    if (command_is_dead ||
                        nread == 0) {         /*  we catched a SIGCHLD,  or slave command has closed its stdout */
                        if (promptlen > 0)    /* commands dying words were not terminated by \n ... */
                            my_putchar('\n'); /* provide the missing \n */
                        cleanup_rlwrap_and_exit(EXIT_SUCCESS);
                    } else if (errno == EINTR) { /* interrupted by signal ...*/
                        continue;                /* ... don't worry */
                    } else if (!seen_EOF) {      /* maybe command has just died (and SIGCHLD, whose handler sets
                                                    command_is_dead is not  */
                ...
                if (skip_rlwrap()) { /* Race condition here! The client may just have finished an emacs session and
                                        returned to cooked mode, while its ncurses-riddled output is stil waiting for us
                                        to be processed. */
                    if (advise_always_readline) {
                        char *newlines = "\n\n";
                        assert(!always_readline);
                        write_patiently(STDOUT_FILENO, newlines, strlen(newlines),
                                        "to stdout"); /* make the following warning stand out ...  */
                    ...
                    write_patiently(STDOUT_FILENO, buf, nread,
                                    "to stdout"); /* ... and print it before the clients output */
                    DPRINTF1(DEBUG_AD_HOC, "advise_always_readline = %d", advise_always_readline);
                    DPRINTF2(DEBUG_TERMIO, "read from pty and wrote to stdout  %d  bytes in direct mode  <%s>", nread,
                             M(buf));
                    yield();
                    continue;
                }
                ...
                if (within_line_edit) /* client output arrives while we're editing keyboard input:  */
                    save_rl_state();  /* temporarily disable readline and restore the screen state before readline was
                                         called */
                ...
                */
                ...
                leave_prompt_alone =
                    *saved_rl_state.raw_prompt ==
                        '\0' /* saved_rl_state.raw_prompt = "" in two distinct cases: when there is actually no prompt,
                ...
                new_output_minus_prompt = process_new_output(
                    buf, &saved_rl_state); /* chop off the part after the last newline and put this in
                                              saved_rl_state.raw_prompt (or append buf if  no newline found)*/

                if (impatient_prompt) { /* in impatient mode, ALL command output is passed through the OUTPUT filter,
                                           including the prompt The prompt, however, is filtered separately at cooking
                                           time and then displayed */
                    ...
                    my_putstr(filtered);
                    free(filtered);
                    if (regexp_means_prompt && prompt_regexp &&
                        match_regexp(saved_rl_state.raw_prompt, prompt_regexp, FALSE)) {
                        /* user specified -O!.... so any natching candidate prompt will be cooked and output
                         * immediately: */
            ...
            if (FD_ISSET(STDIN_FILENO, &readfds)) { /* key pressed */
                unsigned char
                    byte_read; /* the readline function names and documentation talk about "characters" and "keys",
                                  but we're reading bytes (i.e. unsigned chars) here, and those may very well be
                                  part of a multi-byte character. Example: hebrew "aleph" in utf-8 is 0xd790; pressing
                                  this key will make us read 2 bytes 0x90 and then 0xd7, (or maybe the other way round
                                  depending on endianness??) The readline library hides all this complexity and allows
                                  one to just "pass the bytes around" */
                ...
                assert(nread == 1);
                DPRINTF2(DEBUG_TERMIO, "read from stdin: byte 0x%02x (%s)", byte_read,
                         mangle_char_for_debug_log(byte_read, TRUE));
                if (skip_rlwrap()) { /* direct mode, just pass it on */
                    ...
                    if (!user_has_typed_first_NL && (byte_read == '\r' || byte_read == '\n')) {
                        ...
                        advise_always_readline = TRUE; /* first NL is in direct mode: advise the user that she probably
                                                          wants --always-readline */
                    }
                    ...
                    if (term_eof && byte_read == term_eof &&
                        strlen(rl_line_buffer) == 0) { /* hand a term_eof (usually CTRL-D) directly to command */
                        ...
                        *sent_EOF = term_eof;
                        ...
                        DPRINTF2(DEBUG_READLINE, "rl_callback_read_char() (_rl_eof_char=%d, term_eof=%d)", _rl_eof_char,
                                 term_eof);
...
static void init_rlwrap(char *command_line)
{
    ...
    const char *config_dir = get_configdir("rlwrap", ENV_HOME, XDG_CONFIG);
    if (!config_dir) {
        myerror(WARNING | NOERRNO, "No HOME, using '.'");
    }

    /* Determine history file name and check its existence and permissions */
    if (!history_filename) {
        history_filename = create_history_filename(command_name);
    }

    if (write_histfile) {
        if (!is_history_file_writable(history_filename)) {
    ...
    read_history(history_filename); /* ignore errors here: history file may not
            yet exist, but will be created on exit */
    ...
    completion_filename = create_completion_filename(command_name);
    ...
    if (is_file_readable(completion_filename)) {
        feed_file_into_completion_list(completion_filename, FALSE);
    } else if (is_file_readable(default_completion_filename)) {
...
static char *check_optarg(char opt, int remaining, bool isdummy, const char *suggestion)
{
    ...
    if (!optarg)
        last_option_didnt_have_optional_argument = TRUE; /* if this variable is set, and  if command is not found,
                                                            ...
                                                            as optional argument (e.g. 'rlwrap -a password sqlplus' will
                                                            try to execute 'password sqlplus' ) */
...
char *read_options_and_command_name(int argc, char **argv)
{
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'd':
                ...
                my_fopen(&debug_fp, DEBUG_FILENAME, "w+",
                         "debug log"); /* w+, not w: both parent and child write to the same logfile, and need to fseek
                                          beyond where the other may have written stuff */
#else
            ...
            case 'e':
                ...
                if (strlen(extra_char_after_completion) > 1)
                    myerror(FATAL | NOERRNO,
                            "-e (--extra-char-after-completion) argument should be at most one character");
                break;
            case 'E':
                always_echo = TRUE;
                break;
            ...
            case 'F':
                WONTRETURN(myerror(
                    FATAL | NOERRNO,
                    "The -F (--history-format) option is obsolete. Use -z \"history_format '%s'\" instead", optarg));
            case 'g':
...
#endif
                multiline_separator = /* \\\\ will be printed as \\ which is correct if we want ' \ ' to be the
                                         multiline separator */
            ...
            case ':':
                assert(optind > 0);
                WONTRETURN(myerror(FATAL | NOERRNO,
    ...
    if (!complete_filenames &&
        !opt_b) { /* use / and . as default breaking characters whenever we don't complete filenames */
        rl_basic_word_break_characters = add2strings(rl_basic_word_break_characters, "/.");
    }

    if (!complete_filenames && !opt_f && !remember_for_completion &&
        !always_readline) { /* https://github.com/hanslub42/rlwrap/issues/147 */
...
void put_in_output_queue(char *stuff)
{
    ...
    DPRINTF3(DEBUG_TERMIO, "put %d bytes in output queue (which now has %d bytes): %s", (int)strlen(stuff),
             (int)strlen(output_queue), M(stuff));
}
...
void flush_output_queue(void)
{
    ...
    how_much =
        min(BUFFSIZE, nl ? 1 + nl - output_queue
                         : queuelen); /* never write more than one line, and never more than BUFFSIZE in one go */
...
void cleanup_rlwrap_and_exit(int status)
{
    ...
    if (write_histfile &&
        (histsize == 0 || history_total_bytes() > 0)) /* avoid creating empty .speling_eror_history file after typo */
    ...
    if (filter_pid)
        ...
        myerror(WARNING | NOERRNO,
                (filters_killer                     ? "filter was killed by signal %d (%s)"
                 : WEXITSTATUS(filters_exit_status) ? "filter died"
                                                    : "filter exited"),
                filters_killer, signal_name(filters_killer));
    }
...
/* utility functions for config/data file */
/**
 * @brief	 create history filename from Environ variable and command name
 *
 * @param  command_name	 command name
 * @return	 history_filename	created history filename
 */
static char *create_history_filename(const char *command_name)
{
    char *oldtype_histfile = get_oldtype_filename(command_name, "history");

    if (oldtype_histfile && is_file_readable_and_writable(oldtype_histfile)) {
        return oldtype_histfile;
    }

    // create XDG Specs history file name
    const char *histdir = get_configdir("rlwrap", ENV_DATAHOME, XDG_DATA);
    if (histdir == NULL) {  // all environ is not set
        const char *histname = add2strings(".", add3strings(command_name, "_", "history"));
        return add2strings("./", histname);
    }
    return add3strings(histdir, "/history/", command_name);
}

/**
 * @brief  check if history file is writable, and create history dir if not exist
 *
 * @param  history_file path to history file
 * @return bool true:history file is writable / false: not writable
 *
 */
static bool is_history_file_writable(const char *history_file)
{
    if (is_file_exist(history_file)) {
        if (is_file_readable_and_writable(history_file)) {
            return TRUE;
        }
        if (!is_file_mine(history_file)) {
            myerror(WARNING | NOERRNO,
                    "Owner of %s and your effective UID don't match. History "
                    "will be read-only",
                    history_filename);
        } else {
            myerror(FATAL | USE_ERRNO, "cannot read and write %s", history_file);
        }
        return FALSE;
    } else {
        const char *histdir = mydirname(history_file);
        if (is_directory_exist(histdir)) {
            if (!is_directory_writable(histdir)) {
                myerror(FATAL | USE_ERRNO, "cannot create history file %s", history_file);
                return FALSE;
            }
        } else {
            if (!create_writable_directory(histdir)) {
...
/**
 * @brief create completion file name / return old type filename is file is
 * exist.
 *
 * @param const char*	command_name 	command_name (= used for filename prefix
 * @return	 char* created filename for completion
 */
static char *create_completion_filename(const char *command_name)
{
    const char *prefix = "completions";
    char       *oldtype_completion_file = get_oldtype_filename(command_name, prefix);

    if (oldtype_completion_file) {
        return oldtype_completion_file;
    }

    const char *config_dir = get_configdir("rlwrap", ENV_HOME, XDG_CONFIG);
    if (config_dir == NULL) {
        const char *completion_name = add2strings(".", add3strings(command_name, "_", prefix));
        return add2strings("./", completion_name);
    }
    return add3strings(config_dir, "/completions/", command_name);
}

/**
 * @brief	get old typed history / completion file name if files is exist.
 *
 * @param const char*	command_name 	command_name (= used for filename prefix
 * @param const char*	postfix 	history/completion file name postfix
 * (history/completion/ )
 * @return	 char* created filename
 */
static char *get_oldtype_filename(const char *command_name, const char *postfix)
{
    const char *rlwrap_homedir = getenv(ENV_HOME);
    const char *homedir = getenv("HOME");
    const char *filename;
    char       *path = NULL;

    filename = add3strings(command_name, "_", postfix);

    if (rlwrap_homedir) {
        path = add3strings(rlwrap_homedir, "/", filename);
        if (is_file_readable(path)) {
            return path;
        }
    }
    if (homedir) {
        path = add3strings(homedir, "/", add2strings(".", filename));
        if (is_file_readable(path)) {
            return path;
        }
    } else {
        path = add2strings("./", add2strings(".", filename));
        if (is_file_readable(path)) {
            return path;
        }
    }
...
/**
 * @brief get homedir  from Environment variables.
 *     if HOME environment variable is not set, return "." and put error message
 *
 * @param const char*	appdir applicatiob directory for config directory
 * @param const char*   env_name reference environment namr
 * @package xdg_type_t  reference type of XDG Base's Environment variables
 * @return	 char* created filename for completion
 */
static char *get_configdir(const char *appdir, const char *env_name, xdg_type_t xdgtype)
{
    ...
    if ((homedir = getenv(env_name)) != NULL) {
        return homedir;
    }
    switch (xdgtype) {
        case XDG_CONFIG:
    ...
    if (homedir != NULL) {
        return add3strings(homedir, "/", appdir);
    }
    if ((homedir = getenv("HOME")) != NULL) {
        switch (xdgtype) {
            case XDG_CONFIG:
                return add3strings(homedir, "/.config/", appdir);
                break;
            case XDG_DATA:
                return add3strings(homedir, "/.local/share/", appdir);
                break;
            default:
                return add3strings(homedir, "/", add2strings(".", appdir));
                break;
        }
    }
    return NULL;
