...
#define ENV_HOME     "RLWRAP_HOME"
#define ENV_DATAHOME "RLWRAP_DATA"
...
/* in util.c: file & directory check */
extern bool is_directory_exist(const char *dir);
extern bool is_directory_writable(const char *dir);
extern bool is_file_exist(const char *filename);
extern bool is_file_readable(const char *filrname);
extern bool is_file_readable_and_writable(const char *filename);
extern bool is_file_mine(const char *filename);
extern bool create_writable_directory(const char *dir);
...
extern char          *term_backspace, term_eof, term_stop, *term_cursor_hpos, *term_cursor_up, *term_cursor_down,
    *term_cursor_left, *term_smcup, *term_rmcup, *term_rmkx, *term_enable_bracketed_paste,
...
#define KA_SCREECH                                                                                                  \
    kill(getpid(), SIGTRAP); /* enter the debugger - use it to set (conditional) breakpoints from within C code: if \
                                (condition) KA_SCREECH; */

/* DPRINTF0 and its ilk  doesn't produce any output except when DEBUG is #defined (via --enable-debug configure option)
 */

/* Use a superfluous "break" (withn a switch) to prevent "this statement may fall through" warnings when we know that a
 * statement will end the program anyway */
