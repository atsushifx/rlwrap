...
static struct timeval
    *volatile pmy_select_timeout_tv; /* The SIGCHLD handler sets this variable (see zero_select_timeout() below) to make
                                       select() return immediately when a child has died. gcc (4.8 and higher) may
                                       optimize it into a register, which won't work: hence the "volatile" keyword */
...
int my_pselect(int n, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, const struct timespec *ptimeout_ts,
...
void read_patiently2(int fd, void *buffer, int count, int uninterruptible_msec, const char *whence)
{
    ...
    DPRINTF2(DEBUG_AD_HOC, "read_patiently2 tries to read %d bytes (uninterruptible: %d msec)", count,
             uninterruptible_msec);
    ...
    block_all_signals();
    DPRINTF2(DEBUG_AD_HOC, "read_patiently2 read %d bytes: %s", total_read,
             mangle_buffer_for_debug_log(buffer, total_read));
    return;
}
...
void write_patiently2(int fd, const void *buffer, int count, int uninterruptible_msec, const char *whither)
{
    ...
    myalarm(0);
    DPRINTF2(DEBUG_AD_HOC, "write_patiently2 wrote %d bytes: %s", total_written,
             mangle_buffer_for_debug_log(buffer, total_written));
    return;
}
...
void mysetenv(const char *name, const char *value)
{
    ...
    myerror(WARNING | NOERRNO,
            "setting environment variable %s=%s failed, as this system has neither setenv() nor putenv()", name, value);
#endif
...
void set_ulimit(int resource, long value)
{
    ...
    DPRINTF4(DEBUG_ALL, "setrlim() used to set resource #%d to value %ld, result = %d (%s)", resource, value, result,
...
int open_unique_tempfile(const char *suffix, char **tmpfile_name)
{
    ...
    *tmpfile_name = mymalloc(MAXPATHLEN + 1);
...
#else
    {
        ...
        snprintf6(*tmpfile_name, MAXPATHLEN, "%s/%s_%s_%d_%d%s", tmpdir, program_name, command_name, command_pid,
                  tmpfile_counter++, suffix);
...
/* change_working_directory() tries to change rlwrap's working directory to the rlwrapped command's current working
 * directory   */
...
static void print_option(char shortopt, char *longopt, char *argument, int optional, char *comment)
{
    ...
    if (argument) {
        maybe_optional =
            (optional_args && optional ? add3strings("[", argument, "]") : add3strings("  <", argument, ">"));
...
// file, directiry check functions
/**
 * @brief  directory is exist
 *
 * @param	const char	*dir	exist check directory
 * @return	bool TRUE: directory is exist
 */
bool is_directory_exist(const char *dir) { return (access(dir, F_OK) == 0); }

/**
 * @brief  directory is writable
 *
 * @param	const char	*dir	writable check directory
 * @return	bool TRUE: directory is writable
 */
bool is_directory_writable(const char *dir) { return (access(dir, W_OK) == 0); }

/**
 * @brief  check file is exist
 *
 * @param	const char	*filename	exist check file
 * @return	bool TRUE: file is exist
 */
bool is_file_exist(const char *filename) { return (access(filename, F_OK) == 0); }

/**
 * @brief	check file is readable
 *
 * @param	const char	*filename	readable check file
 * @return	bool TRUE: file is readable
 */
bool is_file_readable(const char *filename) { return (access(filename, R_OK) == 0); }

/**
 * @brief check if file is readable and writable
 *
 * @param const char *filename	readable and writable check file
 * @return bool TRUE: file is readable and writable
 */
bool is_file_readable_and_writable(const char *filename) { return (access(filename, R_OK | W_OK) == 0); }

/**
 * @brief check if file is mine
 *
 * @param const char *filename	owner check file
 * @return bool TRUE: file is mine
 */
bool is_file_mine(const char *filename)
{
    struct stat statbuf;

    assert(!stat(filename, &statbuf));
    return (statbuf.st_uid == geteuid());
}

/**
 * @brief	ccreate directory with file is writable
 *
 * @param dir	directory to create
 * @return	bool TRUE: directory is created
 */
bool create_writable_directory(const char *dir)
{
